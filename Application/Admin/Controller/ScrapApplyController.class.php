 ...
 * 物资基础信息处理
 */
class ScrapApplyController extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($status == 'all') {
            $where = "type = 3  and status IN (1,2,3,4)";
        }elseif ( $status == 'do' ) { //已审核
            $where = "type = 3  and status IN (3,4)";
        }else{
            $where = "type = 3  and status IN ({$status})"; 
        }
     ...
     public function ajaxScrapApply()
    {
        ...
        $field='id,order_sn,creater,last_op_time,time,status';
         ...
         if ($status == 'all') {
            ...
            $where = "type = 3  and status = {$status}"; 
        }
    ...
    public function info(){
        ...
        if (IS_POST) {
             ...
             $res_2  = $this->db_m->where( array('apply_id'=>$where['scrap_id'],'receive_id'=>$where['audit_id'],'type'=>70) )->setField('status',2); //更改消息状态
             ...
             if ($res !==false  && $res_2 !==false && $res_3 !==false && $res_4!==false && $res_5!==false) {
                ...
                if ($info['create_id'] ==  $_SESSION['admin']['info']['id']  &&  ($info['status'] == 3 || $info['status'] == 4 ) )  {
                   $this->db_m->where( array('apply_id'=>$id,'receive_id'=>$_SESSION['admin']['info']['id'],'type'=>70) )->setField('status',2); //更改消息状态
                }
