...
class StockInvController extends BaseController
{
    ...
    public function ajaxSv()
    {
     ...
     public function exportExcel($expTitle,$expCellName,$expTableData,$c){
        ...
        $objPHPExcel->getActiveSheet(0)->mergeCells('A1:J2');//合并单元格
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A:J')->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER); //水平居中
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A3:J3')->getFont()->setBold(true);//字体加粗
         ...
         $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G3','出库');

         $objPHPExcel->getActiveSheet(0)->mergeCells('I3:J3');//
         $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I3','库存');
        ...
        // dump($i);exit();
         ...
         //最后单元格设置  
          ...
          $objPHPExcel->getActiveSheet(0)->mergeCells('A'.($i+7).':'.'A'.($i+8));//合并单元格
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.($i+7),'入库合计');
         
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.($i+7),'统计数量');
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.($i+8),'统计金额');

          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.($i+7),$c['in_n']);
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.($i+8),$c['in_t']);



          $objPHPExcel->getActiveSheet(0)->mergeCells('D'.($i+7).':'.'D'.($i+8));//合并单元格
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.($i+7),'出库合计');
         
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.($i+7),'统计数量');
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E'.($i+8),'统计金额');

          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.($i+7),$c['ot_n']);
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.($i+8),$c['ot_t']);




          $objPHPExcel->getActiveSheet(0)->mergeCells('G'.($i+7).':'.'G'.($i+8));//合并单元格
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.($i+7),'出入库差额合计');
         
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.($i+7),'统计数量');
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.($i+8),'统计金额');

          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.($i+7),$c['d_n']);
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.($i+8),$c['d_t']);
