...
class ExchangeController extends BaseController
{
	...
	public function index()
	{
		...
		$w="c.id=g.change_id and g.card_sn=card.order_sn and card.goods_id=bg.id and bg.cate_id=cat.id and c.status= 4 and c.type=3  and card.department=fd.id";
		if ($key_word) {
	...
	public function apply()
	{
		if (IS_POST) {
			...
			if ($card_sn) {
				...
				->where("c.goods_id=bg.id and bg.cate_id=cat.id and c.order_sn = '".$card_sn."' and bg.type_id=3")
				->find();
	...
	public function goodsList()
	{
		...
		->table('k_base_goods bg,k_category cat,k_cards c')
		->join('k_depart d on c.department=d.id ')
		->where("c.goods_id=bg.id and bg.cate_id=cat.id and c.".$order_sn." LIKE '".$value."%' and c.org_id=$org and bg.type_id=3")
		->page(1,C('seachsize'))
		->select();
		$this->ajaxReturn($lists);
	}
	...
	public function auditlist()
	{
		...
		$w="c.id=g.change_id and g.card_sn=card.order_sn and card.goods_id=bg.id and bg.cate_id=cat.id and c.type=3 ";
		if ($goods_name) {
			$w.=" and bg.assets_name like '%{$goods_name}%' ";
		}
		if ($time) {
	...
	public function edit()
	{
		if (IS_POST) {
			...
			$d_msg=M('Message')->where(array('type'=>60,'apply_id'=>$order['id']))->delete();
	...
	public function audit(){
		if (IS_POST) {
			...
			//处理自己的消息表
			$wm=array('type'=>60,'apply_id'=>$order['id'],'receive_id'=>$cur['audit_id']);
			...
			if (($order['status']==3||$order['status']==4) && $order['applicant_id']==$admin['id']) {
				$wm=array('type'=>60,'apply_id'=>$order['id'],'receive_id'=>$admin['id']);
