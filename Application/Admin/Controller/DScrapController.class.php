 ...
 * 物资基础信息处理
 */
class DScrapController extends BaseController
{
    ...
    {   
        ...
        $where ="s.id = sg.scrap_id and sg.card_sn = c.order_sn and c.goods_id = bg.id and bg.cate_id = ca.id and s.status = 4 and s.type =2 ";
    ...
    public function add()
    {
        ...
        if (IS_POST) {
            ...
            // if ( empty($_data[1]['order_sn']) ) { //检测第一个
            //      $this->error('请添加报废信息');
            // }
            ...
            $this->db->startTrans();
             ...
             if ($res && $res_2 && $res_3 && $res_4) {
                 $this->db->commit();
                 $this->success('添加成功',U('DScrap/index'));
             }else{
                $this->db->rollback();
                 $this->error('添加失败');
             }
         ...
         $this->assign('list_dep',$this->getDeparts($org_id));
          ...
          if (isset($_GET['order_sn'])) {
              ...
              $where ="c.goods_id = bg.id and bg.cate_id = ca.id and bg.type_id = 2  and c.order_sn = '{$order_sn}'";
        ...
        if (IS_POST) {
          ...
          // dump($_data);exit();
            ...
            $this->db->startTrans();
             ...
             $d_m = $this->db_m->where( array('apply_id'=>$_data['data']['id'],'type'=>75) )->delete();//清空消息表
             ...
             if ($res!==false && $d_sa!==false && $d_m!==false && $d_sg!==false  && $res_2!==false && $res_3!==false && $res_4!==false) {
                 $this->db->commit();
                 $this->success('修改成功',U('DScrapApply/index'));
             }else{
                $this->db->rollback();
                 $this->error('添加失败');
             }
    ...
    public function getData(){
        ...
        $table = "k_cards c,k_base_goods bg,k_category ca";
        $field="c.order_sn,bg.assets_name,bg.unit,ca.cate_name,c.spec,c.original_value,c.start_date,c.service_life";
        $where ="c.goods_id = bg.id and bg.cate_id = ca.id and bg.type_id = 2  and {$_data['field']} like '{$_data['value']}%'";
