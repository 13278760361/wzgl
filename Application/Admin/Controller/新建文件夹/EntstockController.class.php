...
class EntstockController extends BaseController
{
   ...
   public $outlist=array('order_sn'=>'入库单号','company_name'=>'供应商名称','assets_name'=>'物品名称','spec'=>'规格型号','unit'=>'单位','price'=>'单价','num'=>'数量','total'=>'金额','apply_time'=>'日期','type_name'=>'归类','receipt'=>'经办人');
    ...
    public function index()
    {
        ...
        $where ="e.id = eg.entstock_id and eg.supplier_id = s.id and eg.goods_id = bg.id and bg.type_id = t.id and e.status = 4";
        ...
        if (IS_POST) {
           ...
           if ($this->is_supper) {
            ...
            // dump($_data);exit();
                  ...
                  $res_5 = $this->db_eg->add($dataEG);
                      ...
                      if ( $info =  $this->db_s->where($whereS)->find() ) { //如果存在改物品 加数量和总价
                           ...
                           if ( empty( $val['code'] ) ) { //缺少条形码 
                            $dataS['code']  =  str_repeat(0, 6-strlen( $val['goods_id'] ) ).$val['goods_id'].'-'.str_repeat(0, 6-strlen( $dataEG['supplier_id'] ) ).$dataEG['supplier_id'];                               
                           }else{
                            $dataS['code'] = $val['code'];
                           }
             ...
             if ($res && $res_2 && $res_3 && $res_4 && $res_5 && $res_6 && $res_7 && $res_8) {
                 $this->db->commit();
        ...
        if (IS_POST) {
          ...
          if ($this->is_supper) {
            ...
            $this->db->startTrans();
             ...
             if ($res!==false && $d_s_diffN!==false && $d_s_diffT!==false && $d_eg!==false && $d_ea!==false && $d_m!==false && $d_sd!==false && $res_2!==false && $res_3!==false && $res_4!==false && $res_5!==false && $res_6!==false && $res_7!==false && $res_8!==false) {
                 $this->db->commit();
    ...
    public function getData(){
        ...
        $where ="bg.type_id = t.id and bg.id = s.goods_id and s.supplier_id = su.id  and bg.type_id = 3 and {$_data['field']} like '{$_data['value']}%'";

        $data =  $this->db_bg->table($table)->field($field)->where($where)->limit(10)->select();


        if ( empty($data) ) {
              $table = "k_base_goods bg,k_type t";
              $field="bg.assets_name,bg.unit,t.type_name,bg.id as goods_id";
              $where ="bg.type_id = t.id and bg.type_id = 3 and {$_data['field']} like '{$_data['value']}%'";
