...
class WeixiuController extends BaseController
{
 ...
 public function ajaxindex()
    {
        ...
        if($key)
        {
        	...
        	$w.=' and (k_repair.status=4 or k_repair.status=3) and k_base_goods.type_id=3';
  ...
  //导出数据
	...
	public function goodsList()
	{
		...
		// echo $w;
		$lists=M()
					->field('c.id,c.order_sn,c.store_address,c.spec,c.keeper,de.depart_name,c.original_value,bg.assets_name,bg.unit,cat.cate_name')
					->table('k_cards c,k_base_goods bg,k_category cat,k_depart de')
					->where("c.goods_id=bg.id and bg.cate_id=cat.id and c.department=de.id ".$w." and c.".$order_sn." LIKE '".$value."%' and bg.type_id=3")->limit(20)
					->select();
		// echo $lists;die;
