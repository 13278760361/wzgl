...
class CardsController extends BaseController {
    ...
    //  $where = "c.goods_id=bg.id AND bg.cate_id=cg.id";
        ...
        $count = M('Cards')->table(array('k_cards'=>'c','k_base_goods'=>'bg','k_depart'=>'d'))
                 ->where($where)->count();
        $page = new \Think\PageA($count,20);
        $show           = $page->show();

        $lists = M('Cards')->field('c.*,bg.assets_name,d.depart_name as department')
                 ->table(array('k_cards'=>'c','k_base_goods'=>'bg','k_depart'=>'d'))
                 ->where($where)
    ...
    public function asset_to_trea(){
        ...
        $data = I('get.');
        $len = I('size')?:20;
        $where="ct.outstock_id=og.outstock_id AND og.goods_id=bg.id AND bg.type_id=c.id AND bg.type_id=3 AND ct.status=0";
        if($data['assets_name']!=""){
        ...
        $show           = $page->show();
        $lists = M('cards_temp')
                 ->field("ct.id,bg.assets_name,bg.id as goods_id,c.cate_name,og.num,og.spec")
                 ->table(array('k_cards_temp'=>'ct','k_outstock_goods'=>'og','k_base_goods'=>'bg','k_category'=>'c'))
                 ->limit($page->firstRow.','.$len)
                 ->order('ct.id desc')
                 ->where($where)->select();
